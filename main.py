...
longpoll = VkLongPoll(vk_session, 209012920)


class User:
    def __init__(self, minion_level, id, hp, damage, mode, money, grade, energy, level, count5, count10, count15, coal,
                 pers_name, last_minion_work, minion_evo, axe_level, oak, oak_limit, sklad_level, pick_level):
        self.id = id
        self.hp = hp
        self.damage = damage
        self.mode = mode
...
def check_registration(id):
    members = vk_session.method('groups.getMembers', {'group_id': 209012920})['items']
...
def read_bd():
    ...
    for line in lines:
        line = eval('{' + line + '}')
        if line != '{}':
            users.append(User(minion_level=line['minion_level'], hp=line['hp'], damage=line['damage'],
                              axe_level=line['axe_level'], pick_level=line['pick_level'],
                              ...
                              mode=line['mode'], money=line['money'], grade=line['grade'], energy=line['energy'],
                              level=line['level'], count5=line['count5'], count10=line['count10'],
                              count15=line['count15'], coal=line['coal'], pers_name=line['pers_name']))
    return users
...
reg_key = get_keyboard([
    [('Воин', 'белый'), ('Маг', 'белый'), ('Лучник', 'белый')]
])
...
while True:
    try:
                    ...
                    if msg == 'начать':
                        ...
                        if check_registration(id):
                            ...
                            if not (flag):  # если новый пользователь, то переходим в меню регистрации
                                users.append(User(id=id, hp=50, damage=5, coal=24, last_minion_work=0, sklad_level=0,
                                                  pick_level=0, oak_limit=0, minion_evo=0, oak=0, axe_level=0,
                                                  pers_name=None, minion_level=0, mode='registration1', money=50,
                                                  grade=None, energy=100, level=1, count5=0, count10=0, count15=0))
                                sender(id, 'Выберите ваш класс:', reg_key)
        ...
        with open(f'{tdir(__file__)}/errors.txt'.replace('\\', '/').replace('main3.py/', ''), 'a',
